...
export class NexaDom {
  ...
  constructor(options) {
    ...
    const extractor = oldElement.getAttribute("NexaDom") || elementId; // Ubah default ke elementId
    ...
    function renderData(pageData) {
      requestAnimationFrame(() => {
        try {
          ...
          // Render data
          const items = pageData[extractor] || [];
          ...
          result = `<div class="nx-row">
            ${items
              .map((item, index) => {
                ...
                // Proses template untuk setiap item
                    ...
                    try {
                      ...
                      if (
                        ...
                        const cleanPath = propPath
                          .replace(`${key}.`, "")
                          .replace("row.", "");
                        let value = getNestedValue(item, cleanPath);
                      ...
                      else if (item[propPath] !== undefined) {
                        let value = item[propPath];
